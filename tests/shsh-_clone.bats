...
@test "without arguments prints usage" {
  run shsh-_clone
  assert_failure
  assert_line "Usage: shsh _clone <use_ssh> <site> <package> [<ref>]"
}
...
@test "too many arguments prints usage" {
  run shsh-_clone false site a/b ref fourth_arg
  assert_failure
  assert_line "Usage: shsh _clone <use_ssh> <site> <package> [<ref>]"
}
...
@test "install a specific version" {
  ...
  run shsh-_clone false site username/package version
  assert_success
  assert_output "git clone --depth=1 -b version --recursive https://site/username/package.git ${SHSH_PACKAGES_PATH}/username/package"
}

@test "does nothing if package is already present" {
  mkdir -p "$SHSH_PACKAGES_PATH/username/package"

  run shsh-_clone false github.com username/package
...
@test "using a different site" {
  ...
  run shsh-_clone false site username/package
  assert_success
...
@test "setting SHSH_FULL_CLONE to false, clones a package with depth option" {
...
@test "using ssh protocol" {
  ...
  run shsh-_clone true site username/package
  assert_success
